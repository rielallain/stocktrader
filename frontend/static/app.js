...
function renderSummary() {
  ...
  // Day P/L and total P/L across portfolio holdings (allocation-weighted if set, else equal)
  let dayPL = 0, totalPL = 0, validDay = 0, validTotal = 0;
  for (const s of state.stocks) {
    if (!s.is_portfolio) continue;
    if (s.day_dollar_change != null) { dayPL += s.day_dollar_change; validDay++; }
    if (s.dollar_change != null) { totalPL += s.dollar_change; validTotal++; }
  }
  ...
  const dayEl = document.getElementById('summary-day-pl');
  const totalEl = document.getElementById('summary-total-pl');
  if (validDay) {
    dayEl.textContent = signed(dayPL, 2);
    dayEl.className = 'summary-value ' + gainLossClass(dayPL);
  } else {
    dayEl.textContent = '—';
    dayEl.className = 'summary-value';
  }
  if (validTotal) {
    totalEl.textContent = signed(totalPL, 2);
    totalEl.className = 'summary-value ' + gainLossClass(totalPL);
  } else {
    totalEl.textContent = '—';
